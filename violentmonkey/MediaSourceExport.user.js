// ==UserScript==
// @name MSE video export (experimental)
...
function startFetch(e) {
  ...
  ws.onerror = function() {
    alert('Please run "python daemon.py"');
  };
  ws.onopen = function() {
    videoEle = $('video')[0];
    videoEle.pause();
    
    var fileName = prompt('Input file name: ', document.title);
    if (fileName == null) {
      return;
    }
    var startTime = prompt(`Enter the start time (seconds), total${parseInt(videoEle.duration)} seconds: `, '0');
    if (startTime == null) {
      return;
    }
    if ((startTime = parseFloat(startTime)) == NaN) {
      alert('The time format should be numeric');
      return;
    }
...
function stopFetch(e) {
  ...
  setTimeout(() => {
    ...
    ws.send(desc + '\r\n');
    ws.close();
    ws.addEventListener('close', () => alert('Completed'), {once: true});
  ...
  apply: function(fn, thisArg, [mimeType]) {
    ...
    if (!target.format) {
      ...
      if (storage.audio.format && storage.video.format) {
        var htmls = [];
        htmls.push(`Audio${pad + storage.audio.mimeType}`);
        htmls.push(`Video${pad + storage.video.mimeType}`);
        htmls.push('<a class="start">Click to start exporting (the page cannot be closed)</a>'
          + pad + pad + '<a class="stop">Click to stop exporting</a>');
        var html = htmls.join('<br><br>');
        openDialog('MSE video stream export', html, {
          'start': startFetch,
          'stop': stopFetch,
        });
      }
