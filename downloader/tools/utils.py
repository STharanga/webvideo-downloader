...
def checkFFmpeg():
    with os.popen('ffmpeg -v quiet 2>&1') as f:
        if f.read().strip():
            print('\033[93mWarning!!: ffmpeg command could not be found, merged video might be broken，' + \
                'Please configure PATH，Or put binaries in the current directory（Windows only)\033[0m')
